...
class TrillSensor(object):
    ...
    def __init__(self, i2c, address=0x00, mode=None, sleep=10):
        ...
        self.mode = mode
        self.sleep = sleep / 1000.0  # Convert ms to seconds for CircuitPython
        self.type = 0
    ...
    def _write_reg(self, reg, *values):
        buf = bytearray(1 + len(values))
        buf[0] = reg & 0xFF
        for i, v in enumerate(values):
            buf[i + 1] = v & 0xFF
        self._write(buf)

    def _write_reg_bytes(self, reg, payload):
    ...
    def _read_mem(self, reg, count):
        buf = bytearray(count)
        with self._device as i2c_dev:
            i2c_dev.write(bytes([reg & 0xFF]))
            i2c_dev.readinto(buf)
        return buf

    # Ask the sensor to identify itself and read its type and firmware version
    def identify(self):
        self._write_bytes(REG_DATA)
        time.sleep(0.01)
        self._write_reg(REG_COMMAND, COMMAND_IDENTIFY)
        time.sleep(self.sleep + 0.015)
        buf = self._read(3)
        (_, self.identifiedType, self.firmware) = buf
    ...
    def set_mode(self, mode):
        self.mode = mode
        self._write_reg(REG_COMMAND, COMMAND_MODE, mode)
        time.sleep(self.sleep)
    ...
    def set_scan_settings(self, speed=0, resolution=12):
        ...
        self._write_reg(REG_COMMAND, COMMAND_SCAN_SETTINGS, speed, resolution)
        time.sleep(self.sleep)

    # Update the baseline capacitance values of the sensor
    def update_baseline(self):
        self._write_reg(REG_COMMAND, COMMAND_BASELINE_UPDATE)
        time.sleep(self.sleep)

    # Set the prescaler of the sensor, with
    #  prescaler being a value from 1 to 8
    def set_prescaler(self, prescaler=8):
        self._write_reg(REG_COMMAND, COMMAND_PRESCALER, prescaler)
        time.sleep(self.sleep)
...
class Bar(TrillSensor):
    ...
    def read(self):
        ...
        if self.mode is MODE_CENTROID:
            data = struct.unpack(">10h", self._read_mem(REG_DATA, 4 * self.maxTouches))
        else:
            data = struct.unpack(">26h", self._read_mem(REG_DATA, 2 * self.channels))
...
class Square(TrillSensor):
    ...
    def read(self):
        ...
        if self.mode is MODE_CENTROID:
            data = struct.unpack(">16h", self._read_mem(REG_DATA, 4 * self.directions * self.maxTouches))
        else:
            data = struct.unpack(">30h", self._read_mem(REG_DATA, 2 * self.channels))
...
class Craft(TrillSensor):
    ...
    def read(self):
        ...
        if self.mode is MODE_CENTROID:
            ...
            # In raw mode the Craft sensor exposes 30 channels
            # (2 bytes per channel = 60 bytes total).
            data = struct.unpack(">30h", self._read_mem(REG_DATA, 2 * self.channels))
...
class Ring(TrillSensor):
    ...
    def read(self):
        ...
        if self.mode is MODE_CENTROID:
            ...
            data = struct.unpack(">28h", self._read_mem(REG_DATA, 2 * self.channels))
